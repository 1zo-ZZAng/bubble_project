...
public interface WashingMapper {
	...
	@Select({" SELECT SUBSTRING(rvdate, 0,10) daysales, sum(mprice) total FROM RESERVE WHERE wid=#{wid} AND CONCAT(TO_CHAR(RVDATE, 'YYYY-MM-DD'), ' ', TO_CHAR(rvtime, 'HH:MI')) <= SYSDATE GROUP BY SUBSTRING(rvdate, 0,10) "})
	public List<Map<String, Object>> selectDaySales(@Param("wid") String wid);

	//월매출 - Map 있고
	@Select({" SELECT SUBSTRING(rvdate, 0,7) monthsales, sum(mprice) total  FROM RESERVE WHERE wid=#{wid} AND CONCAT(TO_CHAR(RVDATE, 'YYYY-MM'), ' ', TO_CHAR(rvtime, 'HH:MI')) <= curdate() GROUP BY SUBSTRING(rvdate, 0,7) "})
	public List<Map<String, Object>> selectMonthSales(@Param("wid") String wid);

	//연매출
	@Select({" SELECT SUBSTRING(rvdate, 0,4) yearsales, sum(mprice) total FROM RESERVE WHERE wid=#{wid} AND CONCAT(TO_CHAR(RVDATE, 'YYYY'), ' ', TO_CHAR(rvtime, 'HH:MI')) <= SYSDATE GROUP BY SUBSTRING(rvdate, 0,4) "})
	public List<Map<String, Object>> selectYearSales(@Param("wid") String wid);

	//월별사용자수 => 메인페이지에 띄우기
	@Select({" SELECT SUBSTRING(rvdate, 0,7) monthly, count(*) usercnt FROM RESERVE WHERE wid=#{wid} AND TO_CHAR(RVDATE, 'YYYY-MM-DD HH:MI:ss') <= curdate() GROUP BY SUBSTRING(rvdate, 0,7) "})
